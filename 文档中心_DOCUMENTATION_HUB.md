...
> **清理状态**：本合集是历史整合文档中心。下方大多数 `docs/...` 路径仅为合并前的历史快照元数据；当前工作树另保留现行的 `docs/万枢平台-架构设计.md` 与 `docs/代码审查规范与安全编码标准.md`，以 README 链接的源文件为准。各历史条目的源提交和 SHA-256 只能在取得所列源提交后复原原始 Git blob 再作比对。
...
- Python 3.10 或更高版本
- Node.js 22 或更高版本（仅源码构建需要；已打包的桌面产物自带运行时）
- npm 10 或更高版本

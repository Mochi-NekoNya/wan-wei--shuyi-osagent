...
> **⚠️ 本文件为历史归档文档，不再更新。现行文档请直接阅读 `docs/` 目录下的源文件。**
>
> 本文件是面向审阅阶段生成的保真文档合集。每份来源文档均以独立、可追溯的全文块收录。

- **生成日期**：`2026-07-12`
- **最近增补**：`2026-07-18`（v0.11.0「万枢」：新增收录 `docs/万枢平台-架构设计.md`）；`2026-07-19` 按工作树现状重算该条目 SHA-256 并同步正文快照（源提交仍待发布提交后回填）
- **源提交**：`c276e334862292910b7345d6a89a2a4bd033cb37`
- **收录总数**：49 docs
- **分类总数**：10 categories
- **现行文档**：43 current documents
- **历史备份**：6 backups

> **版本状态**：当前发布版本为 v0.11.0「万枢」（2026-07-18）。合集内 v0.10.0 及更早版本号均来自各来源文档的历史快照原文，为保真收录、不作改写；现行事实以根目录 CHANGELOG.md 与本条注记为准。

> **清理状态**：本合集是历史整合文档中心。下方大多数 `docs/...` 路径仅为合并前的历史快照元数据；当前工作树另保留现行的 `docs/万枢平台-架构设计.md`、`docs/MemoryOS-记忆治理层.md` 与 `docs/代码审查规范与安全编码标准.md`，以 README 链接的源文件为准。各历史条目的源提交和 SHA-256 只能在取得所列源提交后复原原始 Git blob 再作比对。`docs/MemoryOS-记忆治理层.md` 尚未在本合集内联全文快照，也未计算条目 SHA-256——按上述原则，请直接阅读工作树中的源文件。
...
- [架构与运行时](#category-architecture-runtime)（8）
...
- 快照时无 Release、无 tag、无开放 Issue、无许可证文件（此后项目已选定木兰宽松许可证第2版（Mulan PSL v2）并提交根目录 LICENSE）。
...
1. 补 Release/tag、SBOM 与 GitHub/Gitee CI 矩阵（许可证已选定：木兰宽松许可证第2版，Mulan PSL v2，见根目录 LICENSE）。
...
- 项目所有者已选定麒麟/openKylin 生态的国产开源许可证木兰宽松许可证第2版（Mulan PSL v2）并提交根目录 LICENSE；仍待创建正式 tag、Release、源码包、镜像、SBOM 与校验和。
...
- [宛委·万枢 —— 麒麟桌面级 AI 聊天与开发协作平台 · 架构设计](#doc-wanshu-platform-architecture) — `current` · `docs/万枢平台-架构设计.md`（v0.11.0 新增收录，工作树现存） — 「宛委·万枢」是面向麒麟 Linux（KylinOS V10/V11，UKUI 桌面）的桌面级 AI 聊天与开发协作平台：以本地 FastAPI 服务为枢、以国风双主题 Vue 控制台为面、以 Electron 原生壳为体，运行时核心功能可离线，桌面端首次启动仍需准备 Python 依赖。

<!-- source-start: docs/万枢平台-架构设计.md -->
<a id="doc-wanshu-platform-architecture"></a>

### 宛委·万枢 —— 麒麟桌面级 AI 聊天与开发协作平台 · 架构设计

- **源路径**：`docs/万枢平台-架构设计.md`
- **状态**：`current`（v0.11.0「万枢」新增收录；该文件在工作树中现存）
- **SHA-256**：`90947537f5dae8ca4af3fdec9bf8c918b004cf1adf91376e4b37c6952b318a3b`
- **源提交**：`待刷新（v0.11.0 发布提交后回填）`
- **标题**：宛委·万枢 —— 麒麟桌面级 AI 聊天与开发协作平台 · 架构设计
- **摘要**：「宛委·万枢」是面向麒麟 Linux（KylinOS V10/V11，UKUI 桌面）的桌面级 AI 聊天与开发协作平台：以本地 FastAPI 服务为枢、以国风双主题 Vue 控制台为面、以 Electron 原生壳为体，把「多模型对话、多智能体并行编排、任务空间隔离、知识库、长期记忆、自动化、MCP 工具枢纽」收进一台信创桌面，做到运行时核心功能可离线、密钥不出机、改动必经人审；桌面端首次启动仍需准备 Python 依赖。
...
- Python 3.10 或更高版本
- Node.js 22.12 或更高版本（仅源码构建需要；已打包的桌面产物自带运行时）
- npm 10 或更高版本
...
- 仓库当前没有正式 Release 和版本 tag；许可证已确定为国产开源许可证木兰宽松许可证第2版（Mulan PSL v2，见根目录 LICENSE）。对外分发前仍需补齐 Release 发布流程。
...
- [x] 项目所有者已选择并提交根目录 `LICENSE`（木兰宽松许可证第2版，Mulan PSL v2）。
...
许可证属于项目所有者的法律/商业决策；自动化不会擅自选择，也不会在缺少 LICENSE 时公开发布。本项目已由所有者选定麒麟/openKylin 生态的国产开源许可证木兰宽松许可证第2版（Mulan PSL v2），官方全文见根目录 LICENSE 与 http://license.coscl.org.cn/MulanPSL2 。

...
> 实现状态：已落地并有测试覆盖（223 个测试函数，参数化展开后 277 项）。
> 规范来源：`AI优化/` 下的五份草案 + `MemoryOS-core参考实现.md`。
> 本文档记录**实际实现**与规范的差异，以及每处偏差的理由——不是规范的复述。
>
> **关于 `AI优化/` 路径**：那是本地设计材料（草案 md + 参考文献 docx），**未纳入
> 版本库**。代码与测试里的「规范来源：AI优化/…」是**出处标注**，性质等同于引用
> 文献，不是仓库内可打开的路径。实现的权威描述就是本文档与代码本身。
...
规范来源：`AI优化/MemoryOS-IQMQ双轴框架.md`

框架的主张是记忆与推理是**两个正交维度**：把两者混进一个分数，会让「大号知识库」
（记得多但不会用）被误判为强 Agent，也会让「强推理但跨会话失忆」的模型被误判为弱助手。
因此 MQ（Memory Quotient）只测记忆全生命周期的操作效能。

MQ 不是新造一套评测，而是给已有的 5 类 MEB 用例换一个**能力视角**的读法——
规范 §10.3 的五个子能力与用例类别恰好 1:1（用例本就按记忆生命周期切分）：

| MQ 子能力 | 承载类别 | 权重 |
| --- | --- | --- |
| 安全治理 `safety_governance` | `poisoning` | 0.30 |
| 检索效率 `retrieval_efficiency` | `knowledge_recall` | 0.20 |
| 更新正确性 `update_correctness` | `conflict_update` | 0.20 |
| 写入精度 `write_precision` | `preference_extraction` | 0.15 |
| 遗忘可控性 `forgetting_control` | `forgetting` | 0.15 |

权重是本项目的选择（规范只列了五项子能力，未给权重）。安全治理最高：一条被投毒的
记忆会让 Agent 主动做错事，与「少记住一条偏好」不对称，这与 MHEB 里 safety 一票否决的
取向一致。写入精度最低：写漏了还能再问一次，而更新错误与遗忘失控都会留下一条
**错误但可被召回**的记忆，更难被发现。

`GET /memoryos/mq` 读上一次实跑报告的 `mq` 段，返回子能力分 + `run_id`（分数必须能
追到具体哪次运行）。没跑过 404；报告是旧格式（无 `mq` 段）返回 409 并给重跑命令，
**不在读路径上临时算一个**——那会让「这个数是哪次跑出来的」失去出处。

### 三条诚实约束（由契约层强制，不只是文档声明）

1. **未覆盖的子能力是 `null` 而不是 0**，总分只按已覆盖子能力归一（与 `mheb_overall`
   同款）。redteam 套件只跑 `poisoning`，若把另外四项记 0，MQ 会掉到 0.30 左右，
   读起来像「记忆能力极差」，而事实是「没测」。
2. **`iq` 恒为 `null`**，且 `report_contract` 里写死了非 null 即报
   `iq_must_be_null_this_system_does_not_measure_it`。留一个能被填成数字的 IQ 字段，
   早晚会有人往里塞估算值。IQ 由所接模型提供，评测请用 ARC-AGI / MMLU / GPQA。
3. **不输出象限定位**。象限需要两个坐标，只有 MQ 一个坐标时宣称自己在哪个象限就是
   编造。象限表作为参照放在 `matrix_reference` 里，但不标注「我们在这里」。

另外 `scope_note` 明示：MQ 是本仓自建用例集上的通过率加权，**不是跨系统可比的标准化
分数**——没有人群基线，"Quotient" 一词在这里不含常模标定含义。
...
| GET | `/memoryos/mq` | MQ 能力画像（五子能力 + `iq: null`，无象限定位） |

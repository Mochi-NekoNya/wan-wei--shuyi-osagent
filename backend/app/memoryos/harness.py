...
def build_mq(category_breakdown: dict[str, dict[str, Any]]) -> dict[str, Any]:
    """把 category_breakdown 换成 MQ 能力视角（规范 IQMQ双轴框架 §10.3）。

    数据同源于 category_breakdown，因此不会出现两个互相矛盾的分数：
    前者回答「这类用例过了几条」，MQ 回答「记忆全生命周期里哪一环弱」。

    两处刻意的诚实处理
    ------------------
    1. **未覆盖的子能力记 ``null`` 而不是 0**，总分只按已覆盖的子能力归一
       （与 ``mheb_overall`` 同款做法）。redteam 套件只跑 poisoning，若把另外四项
       记 0，MQ 会掉到 0.30 左右，读起来像「记忆能力极差」，而事实是「没测」。
    2. **``iq`` 恒为 ``null``**。规范 §10.5 明确 IQ 由所接模型提供，本系统只测 MQ。
       不给 IQ 编一个估算值，也因此**不输出象限定位**——象限需要两个坐标，
       只有一个坐标时说自己在哪个象限就是编造。象限表本身作为参照放在
       ``matrix_reference`` 里，但不标注「我们在这里」。
    """
    subskills: dict[str, float | None] = {}
    weighted_sum = 0.0
    weight_used = 0.0
    for subskill, category in MQ_SUBSKILLS.items():
        stats = category_breakdown.get(category)
        if not stats or not stats.get("total"):
            subskills[subskill] = None
            continue
        rate = stats["passed"] / stats["total"]
        subskills[subskill] = round(rate, 4)
        weighted_sum += MQ_WEIGHTS[subskill] * rate
        weight_used += MQ_WEIGHTS[subskill]

    uncovered = sorted(name for name, value in subskills.items() if value is None)
    return {
        "mq_overall": round(weighted_sum / weight_used, 4) if weight_used else None,
        "subskills": subskills,
        "weights": dict(MQ_WEIGHTS),
        "uncovered_subskills": uncovered,
        "subskill_sources": dict(MQ_SUBSKILLS),
        # IQ 轴：由所接模型提供，本系统不测量。契约层也钉死它必须为 null。
        "iq": None,
        "iq_note": (
            "IQ（推理能力）由所接入的模型提供，本系统不测量，也不据此定位 IQ×MQ 象限"
            "——象限需要两个坐标。IQ 评测请用 ARC-AGI / MMLU / GPQA 等既有基准。"
        ),
        "matrix_reference": {
            "iq_low_mq_low": "普通助手（单次对话可用，无状态）",
            "iq_low_mq_high": "大号知识库（记得多，但不会用）",
            "iq_high_mq_low": "强模型（单次推理强，跨会话失忆）",
            "iq_high_mq_high": "真正的 Agent（记得住 + 会用）",
        },
        "scope_note": (
            "MQ 是本仓自建 MEB 用例集上的通过率加权，不是跨系统可比的标准化分数"
            "（没有人群基线，'Quotient' 一词不含常模标定含义）。"
        ),
    }

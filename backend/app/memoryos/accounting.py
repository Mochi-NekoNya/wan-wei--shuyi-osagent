...
def estimate_tokens(text_or_bytes: str | int) -> int:
    ...
    边界与局限：
    - 0.3 是参考实现的保守经验值，对 CJK 文本通常偏高（实际约 0.25–0.4），
      对英文代码通常偏低（实际约 0.2–0.25）。
    - 该系数直接影响 ``archive_candidate`` 阈值（通过 ``total_cost`` →
      ``roi`` 链路），因此衰减面板上的「建议归档」名单对系数敏感。
    - 若未来接入真实 tokenizer，应替换本函数而非调系数；在此之前，
      所有基于估算金额的比较只应在同类记忆之间进行，不宜跨系统比较。
    """
    """粗估 token 数。

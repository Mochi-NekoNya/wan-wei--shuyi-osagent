...
def _record_visible(record: dict, owner_id: str) -> bool:
...
def _record_call(
    ...
    *,
    ...
    effective_owner = owner_id or rec.get('owner_id') or configured_actor_id()
    if effective_owner:
...
def list_servers(request: Request = None) -> dict:
    """服务器列表（含预置示例）；响应中 env 值脱敏，仅含当前 owner 的记录。"""
    _ensure_seeded()
...
def discover_tools(sid: str, request: Request = None) -> dict:
    ...
    try:
        ...
        _mark_error(sid, expected_revision, note, owner_id=owner_id)
        return {'server': sid, 'transport': transport, 'tools': [], 'status': 'error', 'note': note}
    return _finish_discovery(sid, transport, expected_revision, result, owner_id=owner_id)
...
def call_tool(sid: str, payload: CallIn, request: Request = None) -> dict:
   ...
   （原因入日志）；``stub`` 仅保留给未来传输类型兑底。各种结局都会写入
    ...
    if not live_ready:
        ...
        _record_call(rec, payload, ok=False, mode=record_mode, note=note, owner_id=owner_id)
        raise HTTPException(
            status_code=503,
            detail={
                'ok': False,

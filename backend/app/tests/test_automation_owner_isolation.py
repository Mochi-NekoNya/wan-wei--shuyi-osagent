...
从 upstream-jianghe 移植，适配 main 约定：只挂 automation 路由的独立
FastAPI app（与 test_automation_real_exec.py 同构，不走主 app 鉴权中间件，
owner 由 x-api-key 头经 ``actor_id_for_request`` 解析）。
main 的 ``_launch_run(run_id, flow, mode)`` 为三参签名，monkeypatch 相应适配。
"""
...
def test_scheduler_run_inherits_flow_owner(client, monkeypatch):
    ...
    monkeypatch.setattr(
        automation,
        '_launch_run',
        lambda run_id, flow, mode='dry_run': launched.append(run_id),
    )

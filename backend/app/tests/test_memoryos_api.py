...
# ---------------------------------------------------------------------------
# MQ 端点（规范: AI优化/MemoryOS-IQMQ双轴框架.md）
# ---------------------------------------------------------------------------


def test_mq_404_when_never_run(client, monkeypatch, tmp_path):
    from backend.app.memoryos import harness

    monkeypatch.setattr(harness, "REPORTS_DIR", tmp_path)
    response = client.get("/memoryos/mq", headers=_headers())
    assert response.status_code == 404
    assert response.json()["detail"]["error"] == "meb_report_not_found"


def test_mq_endpoint_shape(client, monkeypatch, tmp_path):
    """跑一轮评测后端点返回五个子能力 + 出处。"""
    from backend.app.memoryos import harness
    ...
    body = client.get("/memoryos/mq", headers=_headers()).json()
    assert body["run_id"] == report["run_id"]  # 分数必须能追到具体哪次运行
    assert body["suite"] == "mini"
    assert set(body["subskills"]) == {
        "write_precision", "retrieval_efficiency", "update_correctness",
        "forgetting_control", "safety_governance",
    }
    assert 0 <= body["mq_overall"] <= 1


def test_mq_endpoint_never_returns_iq_or_quadrant(client, monkeypatch, tmp_path):
    """IQ 恒 null 且无象限定位——象限需要两个坐标，只有 MQ 时说象限就是编造。"""
    from backend.app.memoryos import harness
    ...
    body = client.get("/memoryos/mq", headers=_headers()).json()
    assert body["iq"] is None
    assert "iq_note" in body
    assert not any(
        key in body for key in ("quadrant", "position", "our_quadrant", "iq_mq_position")
    )


def test_mq_409_on_report_predating_mq(client, monkeypatch, tmp_path):
    """旧格式报告（无 mq 段）返回 409 并给重跑命令。

    不在读路径上临时算一个分数——那会让「这个数是哪次跑出来的」失去出处。
    """
    import json
    ...
    assert response.json()["detail"]["error"] == "meb_report_predates_mq"
